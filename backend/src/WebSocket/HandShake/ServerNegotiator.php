...
namespace Support\WebSocket\HandShake;
...
class ServerNegotiator implements NegotiatorInterface
{
    ...
    public function handshake(RequestInterface $request)
    {
        if (true !== $this->verifier->verifyMethod($request->getMethod())) {
            return new EmptyResponse(405, ['Allow' => 'GET']);
        }
        if (true !== $this->verifier->verifyHTTPVersion($request->getProtocolVersion())) {
            return new EmptyResponse(505);
        }
        ...
        if (true !== $this->verifier->verifyUpgradeRequest($request->getHeader('Upgrade'))) {
            return new EmptyResponse(426, $upgradeSuggestion);
        }
        ...
        return new EmptyResponse(101, array_merge($headers, [
            'Upgrade'              => 'websocket'
          , 'Connection'           => 'Upgrade'
     ...
     */
    public function setSupportedSubProtocols(array $protocols)
    {
